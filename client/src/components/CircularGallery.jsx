...
class Media {
  ...
  createShader() {
    ...
    this.program = new Program(this.gl, {
      ...
      fragment: `
        ...
        void main() {
          ...
          // Fond sombre si l'image n'est pas encore chargée
          vec3 color = mix(vec3(0.12), texColor.rgb, min(1.0, texColor.a + texColor.r + texColor.g + texColor.b));
    ...
    const loadImage = (attempt = 0) => {
      ...
      imgEl.src = attempt === 0 ? this.image : `${this.image}?r=${Date.now()}`
      imgEl.onload = () => {
        texture.image = imgEl
        this.program.uniforms.uImageSizes.value = [imgEl.naturalWidth, imgEl.naturalHeight]
      }
      imgEl.onerror = () => {
        if (attempt < 4) setTimeout(() => loadImage(attempt + 1), 1500 * (attempt + 1))
      }
    }
    loadImage()
  }
  ...
  onResize({ screen, viewport } = {}) {
    ...
    this.plane.scale.x = (this.viewport.width * (465 * this.scale)) / this.screen.width
...
class App {
  constructor(container, { items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase, noScroll, noDrag, autoSpeed, onCenterChange, onItemClick }) {
    ...
    // Démarrer au milieu du tableau dupliqué → boucle seamless dans les deux sens
    if (this.medias && this.medias[0]) {
      const mid = this.medias[0].widthTotal / 2
      this.scroll.current = mid
      this.scroll.target  = mid
  ...
  update() {
    if (this.autoSpeed) this.scroll.target += this.autoSpeed
    ...
    if (this.onCenterChange && this.medias) {
      const centered = this.medias.reduce((best, m) =>
        Math.abs(m.plane.position.x) < Math.abs(best.plane.position.x) ? m : best
      , this.medias[0])
      const idx = centered.index % this.originalLength
      if (idx !== this._lastCenter) {
        this._lastCenter = idx
        this.onCenterChange(idx)
      }
    }
  ...
  handleClick() {
    if (this.onItemClick && this._lastCenter >= 0) this.onItemClick(this._lastCenter)
  }
  ...
  destroy() {
    ...
    if (this.onItemClick) this.container.removeEventListener('click', this.handleClick)
...
export default function CircularGallery({ items, bend = 3, borderRadius = 0.05, scrollSpeed = 2, scrollEase = 0.05, noScroll = false, noDrag = false, autoSpeed = 0, onCenterChange, onItemClick, className = '' }) {
  ...
  useEffect(() => {
    ...
    const app = new App(el, { items, bend, textColor: color, borderRadius, font, scrollSpeed, scrollEase, noScroll, noDrag, autoSpeed, onCenterChange, onItemClick })
    return () => app.destroy()
  }, [items, bend, borderRadius, scrollSpeed, scrollEase, noScroll, noDrag, autoSpeed, onCenterChange, onItemClick])

  return (
    <div
      ref={containerRef}
      className={`w-full h-full overflow-hidden ${onItemClick ? 'cursor-pointer' : noDrag ? 'cursor-default' : 'cursor-grab active:cursor-grabbing'} text-warm font-bold text-[30px] ${className}`}
    />
  )
}
