                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {trophies.filter((t) => t.category === cat).map((t) => (
                        <div key={t.id}
                          ...
                          <span className={`text-2xl ${!t.unlocked ? 'grayscale opacity-40' : ''}`}>{t.unlocked ? t.icon : (t.secret ? <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 text-warm/30" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="11" width="18" height="11" rx="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg> : t.icon)}</span>
